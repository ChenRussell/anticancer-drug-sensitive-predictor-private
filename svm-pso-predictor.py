...
class PSO():
    def __init__(self, pN, dim, max_iter):
        ...
        self.K = 2 / (abs(2 - self.L - cmath.sqrt(self.L ** 2 - 4 * self.L)))
    ...
    def init_Population(self):
        for i in range(self.pN):
            for j in range(self.dim):
                self.X[i][j] = random.uniform(0, 10)
                self.V[i][j] = random.uniform(0, 10)
    ...
    def iterator(self):
        ...
        for iter in range(self.max_iter):
            ...
            # 更新惯性权重
            # self.w = self.wS - (self.wS - self.wE) * iter / MAX_ITER
        return fitness
