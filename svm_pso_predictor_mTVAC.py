...
class PSO_MTVAC():
    def __init__(self, max_iter, x_train, y_train, x_test, y_test, pN=30, dim=2):
        ...
        self.pm = random.uniform(0, 1)  # 突变概率
        self.k = random.randint(0, 29)  # 随机选择一个微粒(index)
        self.j = random.randint(0, 1)  # 随机选择一个维度(index)
        self.m = 2  # 常量, 怎么取值？？？
    ...
    def iterator(self):
        ...
        for iter in range(self.max_iter):
            ...
            if len(fitness) > 2 and (fitness[-1] - fitness[-2] <= 0):
                if self.r1 < self.pm:
                    if self.r2 < 0.5:
                        self.V[self.k][self.j] += self.r3 * self.max_v / self.m
                    else:
                        self.V[self.k][self.j] -= self.r4 * self.max_v / self.m
