...
for run in range(3):
    ...
    pso_TVAC = PSO_TVAC(max_iter=MAX_ITER, data_X=X, data_y=y, n_splits=n_splits)  # 维度代表变量的个数
    pso_TVAC.init_Population()
    fitness_TVAC = pso_TVAC.iterator()

    pso_mTVAC = PSO_MTVAC(max_iter=MAX_ITER, data_X=X, data_y=y, n_splits=n_splits)  # 维度代表变量的个数
    ...
    # pso_hTVAC = PSO_HTVAC(max_iter=MAX_ITER, data_X=X, data_y=y, n_splits=n_splits)  # 维度代表变量的个数
    # pso_hTVAC.init_Population()
    # fitness_hTVAC = pso_hTVAC.iterator()

    pso_mhTVAC = PSO_MHTVAC(max_iter=MAX_ITER, data_X=X, data_y=y, n_splits=n_splits)  # 维度代表变量的个数
    pso_mhTVAC.init_Population()
    fitness_mhTVAC = pso_mhTVAC.iterator()

    pso_original = PSO(max_iter=MAX_ITER, data_X=X, data_y=y, n_splits=n_splits)  # 维度代表变量的个数
