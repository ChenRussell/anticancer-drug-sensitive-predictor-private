...
data = data.drop('Name', axis=1)  # 删除Description列, 改为删除Name列
cols = data.columns
print(cols)

# data = data.pivot(index='Name')
data.index = data['Description'].tolist()  # 将Name列转换为索引列， 改为Description
data = data.drop('Description', axis=1)    # 删除Name列, 改为Description
...
print(data_unstack_select)
data_unstack_select.to_csv('data/drug_cell/drug/Irinotecan_train_data.csv',index=False, float_format='%.2f')