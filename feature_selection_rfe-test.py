...
data = pd.read_csv('data/drug_cell/drug/17-AAG_train_data-rfe.csv')  # fn=7  0.8095

data = data.fillna(0)
x = data.iloc[:, :-1]
y = data.iloc[:, -1]
svc = SVC(kernel="linear", C=1)
# rfe = SA_RFE(estimator=svc, n_features_to_select=4, step=1)
...
x_new['label'] = y
# print(x_new)
x_new.to_csv('data/drug_cell/drug/17-AAG_train_data-rfe-test11.csv', index=False, float_format='%.2f')
